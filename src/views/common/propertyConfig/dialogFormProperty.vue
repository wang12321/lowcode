      ...
      <el-collapse-item v-for="(item,index) in formOptionsList" :key="item.id" v-dragging="{ item: item, list: formOptionsList, group: 'item' }" :title="item.title" :name="index+1">
        <div class="table">
          ...
          >
            ...
            <template v-slot:isRule>
              <div>
                <el-radio-group v-model="radioRule" size="small" @change="onRule">
...
export default {
  ...
  computed: {
    formOptionsList() {
      const formOptions = this.$store.state.tableConfig.addConfig.DialogForm.computed.formList
      return formOptions
    },
    /*
 ...
 *       @param {Boolean} disabled      选传，disabled 默认关闭
    * */
    formList() {
      const formListArr = [{ title: '名称', key: 'title', type: 'input', clearable: true },
        ...
        { title: '是否必填', clearable: true, key: 'isRule', type: 'slotFormItem', options: [{ value: 'true', label: 'true' }, { value: 'false', label: 'false' }], label: 'label', value: 'value', change: this.onRule },
  ...
  methods: {
    ...
    changeCollapse(index) {
      if (Number(index) > 0) {
        this.indexData = 0
        this.formData = this.$store.state.tableConfig.addConfig.DialogForm.computed.formList[Number(index) - 1]
        this.radioRule = this.formData.isRule
      } else {
        this.formData = {}
        this.radioRule = false
      }
    },
    changeAction(item) {
    },
